...
# for img_type in ['tcrb']:
    ...
    spf.save_combined_spectrum_to_csv(img_type, new_folder_path, spectrum_values, extracted_spectrum_list)    
...
for img_type in ['piser','tcrb']:
    ...
    all_peaks = spf.detect_many_peaks(img_type, new_folder_path, height_factor=0.02, min_distance=10, plotCheck=False) # 아크 피크 검출
    knownwavelength_to_pixel = spf.trans_wavelength_positions(img_type, new_folder_path, known_wavelength_list, a, b, all_peaks, plotCheck=False) # 알려진 파장과 검출된 피크를 매칭칭
    ...
    fitting_coeffs, errors = spf.fit_wavelength_pixel(wavelength_N_pixel_pair, degree=2, result_all_polyfit=False, plotCheck_error=False)
    wavelength_positions, spectrum_data = spf.convert_pixel_to_wavelength(img_type, new_folder_path, fitting_coeffs)
    new_wavelengths, resampled_flux_density = spf.plot_resampled_spectra_list(wavelength_positions, spectrum_data, 
                                                                              wl_step=1.0, wl_start=None, wl_end=None, show=False, plot_integration_comparison=True)
    spf.save_resampled_data_to_csv(img_type, new_folder_path, new_wavelengths, resampled_flux_density)
    # resolutions_list, sigmas_list = spf.calculate_spectral_resolution(img_type, new_folder_path, fitting_coeffs, first_matching_wavelength_index, 
