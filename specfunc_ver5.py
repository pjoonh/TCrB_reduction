...
def compute_spectrum(img_type, new_folder_path, median_y_data, fwhm_data, background_values, fwhm_spectrum=2.0, plotCheck=False):
    list_file_path = os.path.join(new_folder_path, f'geo_{img_type}.list')
...
def trim_spectrum_data(spectrum_values, delete_data, plotCheck=False):
...
def extract_arc_spectrum(img_type, new_folder_path, median_y_data, fwhm_data, plotCheck=False):
        ...
        with fits.open(arc_file_path) as hdul:
            ...
            for (star_file, median_y), (_, segmented_fwhm) in zip(median_y_data, fwhm_data):
                ...
                for x in range(arc_img.shape[1]):
                    ...
                    lower_bound = int(median_y - (fwhm_rep / 2))
                    upper_bound = int(median_y + (fwhm_rep / 2))
...
def save_spectrum_data_to_Excel(img_type, new_folder_path, spectrum_values, extracted_spectrum_list):
    # 첫 번째 파일: spectrum_values 저장
    spectrum_dict = {os.path.basename(file_path): spectrum for file_path, spectrum in spectrum_values}
    df_spectrum = pd.DataFrame.from_dict(spectrum_dict, orient='index')
    df_spectrum.columns = [f'Pixel {i}' for i in range(df_spectrum.shape[1])]
    spectrum_file_path = os.path.join(new_folder_path, f'{img_type}_spectrum.xlsx')
    df_spectrum.to_excel(spectrum_file_path)
    print(f"Spectrum values saved to {spectrum_file_path}")
    
    # 두 번째 파일: extracted_spectrum_list 저장
    with pd.ExcelWriter(os.path.join(new_folder_path, f'arc_{img_type}_spectrum.xlsx'), engine='xlsxwriter') as writer:
        row_pos = 0
        previous_arc_file = None  # arc_file 변경 감지
        first_arc_file = True  # 첫 번째 arc_file에만 x축 레이블 추가

        writer.book.add_worksheet('Arc Spectra')
        worksheet = writer.sheets['Arc Spectra']

        for arc_file, star_file, extracted_spectrum in extracted_spectrum_list:
            if arc_file != previous_arc_file:
                # arc_file이 변경될 때마다 한 줄 띄우기
                if previous_arc_file is not None:
                    row_pos += 1

                previous_arc_file = arc_file  # 현재 arc_file 기억

                # x축 픽셀 레이블 추가
                worksheet.write(row_pos, 0, 'Star File')
                for col, col_name in enumerate([f'Pixel {i}' for i in range(len(extracted_spectrum))]):
                    worksheet.write(row_pos, col + 1, col_name)

                row_pos += 1  # 다음 데이터로 이동

            arc_name = os.path.basename(arc_file)
            star_name = os.path.basename(star_file)

            star_dict = {star_name: extracted_spectrum}
            df_arc = pd.DataFrame.from_dict(star_dict, orient='index')

            # Arc Spectra 시트에 저장
            df_arc.to_excel(writer, sheet_name='Arc Spectra', startrow=row_pos, index=True, header=False)

            row_pos += df_arc.shape[0]  # 다음 데이터를 위해 한 줄 증가

    print(f"Extracted spectrum data saved to arc_{img_type}_spectrum.xlsx")