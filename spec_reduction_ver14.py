...
for img_type in ['piser','tcrb']:
    ...
    fitting_coeffs, errors = spf.fit_wavelength_pixel(wavelength_N_pixel_pair, degree=2, result_all_polyfit=False, plotCheck_error=True)
    wavelength_positions, spectrum_data = spf.convert_pixel_to_wavelength(img_type, new_folder_path, fitting_coeffs)
    print(wavelength_positions[0])
    input('???????????????????')
