...
# 2차 매칭 : adjust된 파장의 부근에서 가우시안 --> 정확한 뮤값 찾고 매칭 ///// 그리고 파장별로 sigma 계산 및 플롯 - 분해능 관련
def refine_pixel_positions(img_type, new_folder_path, adjusted_pixel_positions, search_range=20, plotCheck=False, plotCheck2_1=False, plotCheck2_2=False):
    ...
    for i, fits_file in enumerate(fits_files):
        ...
        for adj_pixel in adjusted_pixel_positions[i]:
            if adj_pixel is None:
                ...
                mu_sigma_list.append((None, None))
                continue
        ...
        mu_sigma_list_all.append(mu_sigma_list)
        ...
        if plotCheck2_1:
            ...
            ax1.plot(spectrum_data, label='Extracted Arc Spectrum', color='blue')
            ax1.set_xlabel('Pixel', color='black')
            ax1.set_ylabel('Intensity', color='black')
            ax1.tick_params(axis='y', labelcolor='black')
            
            valid_values = [(mu, sigma) for mu, sigma in mu_sigma_list if mu is not None and sigma is not None]
            if valid_values:
                mu_values, sigma_values = zip(*valid_values)
                ax2 = ax1.twinx()
                ax2.scatter(mu_values, sigma_values, color='red', alpha=0.6)
                ax2.set_ylabel('Sigma', color='black')
                ax2.tick_params(axis='y', labelcolor='black')
                
                for mu in mu_values:
                    ax1.axvline(x=mu, color='gray', linestyle='dotted', alpha=0.7)
            
            # legend
            plt.plot([], [], color='blue', label='Extracted Arc Spectrum')
            plt.plot([], [], marker='o', color='w', markerfacecolor='red', markersize=6, alpha=0.6, label='Sigma Values')
            plt.plot([], [], color='gray', linestyle='dotted',  alpha=0.7, label='ReAdjusted Pixel')
            plt.legend()

            plt.title(f'Gaussian Fit: Mu vs Sigma in {os.path.basename(fits_file)}')
            plt.grid()
            plt.show()

        # mu - sigma 플롯(원본 데이터 표기 없이 sigma만만)
        if plotCheck2_2:
            plt.figure(figsize=(10, 6))
            plt.xlabel('Pixel', color='black')
            plt.ylabel('Sigma', color='black')
            plt.tick_params(axis='y', labelcolor='black')
            
            valid_values = [(mu, sigma) for mu, sigma in mu_sigma_list if mu is not None and sigma is not None]
            if valid_values:
                mu_values, sigma_values = zip(*valid_values)
                plt.scatter(mu_values, sigma_values, color='red', alpha=0.6)
                
                for mu in mu_values:
                    plt.axvline(x=mu, color='gray', linestyle='dotted', alpha=0.7)
    ...
    return readjusted_pixel_positions, mu_sigma_list_all

# 분해능 계산 함수
def calculate_resolution(img_type, new_folder_path, mu_sigma_list_all, plotCheck=False):
    resolution_data = []
    ...
    for idx, (mu_sigma_list, fits_file) in enumerate(zip(mu_sigma_list_all, fits_files)):
        resolution_list = []
        for mu, sigma in mu_sigma_list:
            if mu is not None and sigma is not None and sigma > 0:
                resolution = mu / (2.355 * sigma)  # 분해능 계산
                resolution_list.append((mu, resolution))
            else:
                resolution_list.append((None, None))
        resolution_data.append(resolution_list)
    
        if plotCheck:
            plt.figure(figsize=(10, 6))
            plt.xlabel('Pixel')
            plt.ylabel('Resolution')
            plt.title(f'Spectral Resolution across Wavelength - {os.path.basename(fits_file)}')
            plt.grid()
            
            valid_values = [(mu, res) for mu, res in resolution_list if mu is not None and res is not None]
            if valid_values:
                mu_values, resolution_values = zip(*valid_values)
                plt.scatter(mu_values, resolution_values, color='blue', alpha=0.6)
            
            plt.plot([], [], marker='o', color='w', markerfacecolor='blue', markersize=6, alpha=0.6, label='Spectral Resolution')
            plt.legend()
            plt.show()
...
def fit_wavelength_pixel(multi_image_data, degree=2, result_all_polyfit=False):
    all_coeffs = []
    ...
    for img_idx, data in enumerate(multi_image_data):
        ...
        if result_all_polyfit:
            coeffs_dict = {}
            for d in range(1, 6):  # 1차부터 5차까지 모두 보여줌
            ...
            coeffs = np.polyfit(wavelengths, pixels, degree) # 지정된 차수에 대해서만 보여줌
            poly_eq = " + ".join(f"{c:.6e}*x**{i}" for i, c in enumerate(reversed(coeffs)))
