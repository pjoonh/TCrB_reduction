...
for img_type in ['piser', 'tcrb']:
    ...
    fwhm_data = spf.compute_fwhm(img_type, new_folder_path, median_y_data, sigma=2, n_segments=15, plotCheck=False)
    ...
    spectrum_values = spf.compute_spectrum(img_type, new_folder_path, median_y_data, fwhm_data, background_values, fwhm_spectrum=4, plotCheck=False, plotCheck2=False)
...
# 9. wavelength calibration
for img_type in ['piser', 'tcrb']:
    spf.wavelength_equation(img_type, new_folder_path, sigma=2, prominence_factor=0.05, plotCheck=True, resultPrint=False)
