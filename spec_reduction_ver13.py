...
for img_type in ['piser', 'tcrb']:
    ...
    second_matching_wavelength_index, mu_sigma_list_all = spf.refine_pixel_positions(img_type, new_folder_path, first_matching_wavelength_index, search_range=20, 
                                                                  plotCheck=False, plotCheck2_1=False, plotCheck2_2=False)
    resolution_data = spf.calculate_resolution(img_type, new_folder_path, mu_sigma_list_all, plotCheck=True)
    wavelength_N_pixel_pair = spf.pair_wavelengths_with_pixels(known_wavelength_list, second_matching_wavelength_index)
    fitting_coeffs = spf.fit_wavelength_pixel(wavelength_N_pixel_pair, degree=2, result_all_polyfit=False)
