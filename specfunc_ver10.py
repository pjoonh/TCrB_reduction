...
def mark_wavelength_positions(img_type, new_folder_path, wavelength_list, a, b, plotCheck=False):
    """
    주어진 일차식 (Wavelength = a * Pixel + b) 을 이용하여 특정 파장값에 해당하는 픽셀 위치를 계산하고 플롯에 표시.
    
    Parameters:
        img_type (str): 이미지 타입
        new_folder_path (str): 리스트 파일이 저장된 폴더 경로
        wavelength_list (list of float): 표시할 파장값 리스트
        a (float): 선형 변환 계수 (픽셀 -> 파장)
        b (float): 선형 변환 절편
    """
    list_files = sorted(glob.glob(os.path.join(new_folder_path, f'extrac_arc*_{img_type}.list')),
                         key=lambda x: int(''.join(filter(str.isdigit, os.path.basename(x)))))
...
def detect_many_peaks(img_type, new_folder_path, sigma=1, height_factor=0.05, min_distance=10, plotCheck=False):
    """
    가우시안 스무딩을 적용한 후 작은 피크까지 모두 검출하는 함수.
    
    Parameters:
        img_type (str): 이미지 타입
        new_folder_path (str): 리스트 파일이 저장된 폴더 경로
        sigma (float): 가우시안 필터의 시그마 값 (기본값: 1)
        height_factor (float): 피크 검출 임계값 (최대값의 비율, 기본값: 0.2)
        min_distance (int): 피크 간 최소 거리 (기본값: 5)
    """
    list_files = sorted(glob.glob(os.path.join(new_folder_path, f'extrac_arc*_{img_type}.list')),
                         key=lambda x: int(''.join(filter(str.isdigit, os.path.basename(x)))))
    ...
    for fits_file in fits_files:
        ...
        # 피크 검출
        peak_threshold = max(smoothed_spectrum) * height_factor
        peaks, _ = find_peaks(smoothed_spectrum, height=peak_threshold, distance=min_distance)
        
        # 결과 플로팅
        if plotCheck:
            plt.figure(figsize=(10, 5))
            plt.plot(smoothed_spectrum, label='Smoothed Spectrum')
            plt.scatter(peaks, smoothed_spectrum[peaks], color='orange', marker='o', label='Detected Peaks')
...
# 9-2) 파장 재보정
def corrected_wavelength_positions(img_type, new_folder_path, wavelength_positions, peaks, sigma=1, peaks_radius=4, plotCheck=False):
    corrected_positions = []
    
    for wavelength_pos in wavelength_positions:
        nearby_peaks = [p for p in peaks if abs(p - wavelength_pos) <= peaks_radius]
        
        if nearby_peaks:
            corrected_positions.append(min(nearby_peaks, key=lambda x: abs(x - wavelength_pos)))
        else:
            corrected_positions.append(wavelength_pos)
    
    if plotCheck:
        fits_files = sorted(glob.glob(os.path.join(new_folder_path, f'extrac_arc*_{img_type}.list')),
                            key=lambda x: int(''.join(filter(str.isdigit, os.path.basename(x)))))
        ...
        fits_file_list = []
        for list_file_path in fits_files:
            with open(list_file_path, 'r') as f:
                fits_file_list.extend([line.strip() for line in f.readlines() if line.strip()])
        
        if not fits_file_list:
            print("No FITS files found in the list files.")
            return corrected_positions
        
        fits_file = fits_file_list[0]  # 첫 번째 파일만 사용
        
        with fits.open(fits_file) as hdul:
            spectrum_data = hdul[0].data
        
        # 가우시안 스무딩 적용
        smoothed_spectrum = gaussian_filter1d(spectrum_data, sigma)
        
        plt.figure(figsize=(12, 6))
        plt.plot(smoothed_spectrum, label='Smoothed 1D Spectrum', color='blue')
        plt.scatter(peaks, smoothed_spectrum[peaks], color='orange', marker='o', label='Detected Peaks')
        plt.scatter(wavelength_positions, 
                    spectrum_data[np.array(wavelength_positions, dtype=int)], 
                    color='red', marker='x', label='Initial Wavelength Positions')
        plt.scatter(corrected_positions, 
                    spectrum_data[np.array(corrected_positions, dtype=int)], 
                    color='green', marker='s', label='Corrected Wavelength Positions')
        
        plt.xlabel('Pixel')
        plt.ylabel('Intensity')
        plt.title(f'Peak Correction for {os.path.basename(fits_file)}')
        plt.legend()
        plt.grid()
        plt.show()
