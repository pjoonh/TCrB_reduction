    ...
    with open(list_file_path, 'r') as file:
        for line in file:
            ...
            with fits.open(file_path) as hdul:
                ...
                # Rotate the image
                geometry_img = rotate(img, total_angle, reshape = False, cval = 0)
...
def calculate_background_from_median_y(img_type, new_folder_path, median_y_data):
    background_values = []
    ...
    for file_path, median_y in median_y_data:
        ...
        with fits.open(file_path) as hdul:
            ...
            # Extract the column corresponding to median_y
            col_data = img[:, median_y]
            # Find the y-coordinate with the maximum pixel value in the column
            peak_y = np.argmax(col_data)
            peak_value = col_data[peak_y]
            
            # Define the half maximum value
            half_max = peak_value / 2
            
            # Find top and bottom indices where the value drops below the half maximum
            top_idx = np.where(col_data[:peak_y] < half_max)[0]
            top_idx = top_idx[-1] if top_idx.size > 0 else 0

            bottom_idx = np.where(col_data[peak_y:] < half_max)[0]
            bottom_idx = peak_y + bottom_idx[0] if bottom_idx.size > 0 else img.shape[0] - 1
            
            # Calculate FWHM
            fwhm = bottom_idx - top_idx
            
            # Define the background pixel value as the median of the column excluding the FWHM region
            background_region = np.concatenate([col_data[:top_idx], col_data[bottom_idx + 1:]])
            background_value = np.median(background_region)

            # Append the result
            background_values.append((file_path, background_value))

    return fwhm, background_values

def process_and_plot_spectrum_data(img_type, new_folder_path, median_y_data, fwhm, background_values):
    for idx, (file_path, median_y) in enumerate(median_y_data):
        # Open the FITS file
        with fits.open(file_path) as hdul:
            img = hdul[0].data  # Get the image data

            # Calculate the range for FWHM * 3 around median_y
            fwhm_range = int(3 * fwhm)
            y_min = max(0, median_y - fwhm_range)
            y_max = min(img.shape[0], median_y + fwhm_range + 1)

            # Extract the region of interest
            roi = img[y_min:y_max, :]  # Rows within FWHM range, all columns

            # Sum pixel values in the region of interest for each column
            summed_values = np.sum(roi, axis=0)

            # Get the background value for this file from the background_values list
            background_value = float(background_values[idx][1])  # background_value is the second element of the tuple

            # Calculate the background contribution for the region of interest
            background_contribution = background_value * (2 * fwhm_range)

            # Subtract background contribution from summed values
            processed_data = summed_values - background_contribution

            # Plot the processed data
            plt.figure(figsize=(10, 6))
            plt.plot(processed_data, label="Processed Spectrum", color="blue")
            plt.axhline(0, color="red", linestyle="--", label="Baseline (Background Subtracted)")
            plt.title(f"Processed Spectrum for {file_path}")
            plt.xlabel("Column Index (x)")
            plt.ylabel("Intensity (Summed Values - Background)")
            plt.legend()
            plt.grid(True)
            plt.show()
...
def extract_arc_spectrum(img_type, new_folder_path, median_y_data, dH, LV=False, PV=False):
    # Path to the .list file
    list_file_path = os.path.join(new_folder_path, f'geo_{img_type}.list')

    # Read file paths from the .list file
    with open(list_file_path, 'r') as list_file:
        arc_file_paths = [line.strip() for line in list_file if line.strip()]

    processed_data = []

    # Match Arc images to the median_y values from standard/target images
    for arc_file_path, (std_file_path, median_y) in zip(arc_file_paths, median_y_data):
        with fits.open(arc_file_path) as hdul:
            img = hdul[0].data  # Get the Arc lamp image data

            # Calculate the range based on median_y and dH
            lower_bound = max(0, median_y - dH)
            upper_bound = min(img.shape[0] - 1, median_y + dH)

            # Extract the data within the y-range
            y_range_data = img[lower_bound:upper_bound+1, :]
            median_values = np.median(y_range_data, axis=0)  # Calculate median along y-direction (rows)

            # Append the result as (Arc file path, 1D array of median values)
            processed_data.append((arc_file_path, median_values))

            # Visualize the image with horizontal lines
            if LV:
                show_image_with_line(img, median_y, arc_file_path)

            # Visualize the 1D spectrum data
            if PV:
                show_1d_spectrum(img, median_values, arc_file_path)

    # Prepare the DataFrame for CSV output
    all_data = []
    for arc_file_path, median_values in processed_data:
        file_data = {"File Path": arc_file_path}
        file_data.update({f"X_{i}": median_values[i] for i in range(len(median_values))})
        all_data.append(file_data)

    # Save the data to CSV
    csv_file_path = os.path.join(new_folder_path, f"{img_type}_extracted_data.csv")
    df = pd.DataFrame(all_data)
    df.to_csv(csv_file_path, index=False)

    print(f"Extracted Arc lamp data saved as {csv_file_path}")
    
    
def show_1d_spectrum(img, median_values, file_path):
    plt.figure(figsize = (10, 5))
    
    # Plot the 1D Spectrum
    plt.plot(median_values, label = '1D Spectrum', color = 'blue')
    plt.title(f'1D Spectrum of {file_path}')
