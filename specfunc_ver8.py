...
def compute_fwhm(img_type, new_folder_path, median_y_data, sigma=2, n_segments=15, plotCheck=False):
    list_file_path = os.path.join(new_folder_path, f'geo_{img_type}.list')
    ...
    for file_path, median_y in median_y_data:
        with fits.open(file_path) as hdul:
            ...
            for x in range(img.shape[1]):  # Iterate through each column
                ...
                fit_range = 10  # Adjust as needed
                y_fit = np.arange(max(0, peak_y - fit_range), min(len(column_data), peak_y + fit_range))
                x_fit = smoothed_column[y_fit]
                
                # Initial guess for Gaussian parameters
                A_guess = np.max(x_fit)
                mu_guess = peak_y
                sigma_guess = 3  # A rough initial guess for sigma
                
                try:
                    popt, _ = curve_fit(fwhm_gaussian, y_fit, x_fit, p0=[A_guess, mu_guess, sigma_guess])
                    A_fit, mu_fit, sigma_fit = popt
                    fwhm = 2.355 * abs(sigma_fit)  # Convert sigma to FWHM
                except:
                    fwhm = None
                
                fwhm_values.append(fwhm)
            
            # Compute segment-wise median FWHM
            ...
            for i in range(n_segments):
                ...
                segment_fwhm_values.append(median_fwhm)
            
            fwhm_data.append((file_path, segment_fwhm_values))
            ...
            if plotCheck: 
                ...
                # Overlay FWHM lines
                for x, fwhm in enumerate(fwhm_values):
                    if fwhm is not None:
                        y_min = median_y - fwhm / 2
                        y_max = median_y + fwhm / 2
                        plt.plot([x, x], [y_min, y_max], color='red', alpha=0.3, linewidth=1.5)
            
                # Overlay segment-wise FWHM
                for i, median_fwhm in enumerate(segment_fwhm_values):
                    if median_fwhm is not None:
                        x_center = (i + 0.5) * segment_size
                        y_min = median_y - median_fwhm / 2
                        y_max = median_y + median_fwhm / 2
                        plt.fill_betweenx([y_min, y_max], x_center - segment_size // 2, x_center + segment_size // 2, color='white', alpha=0.8)
    ...
    for (file_path, median_y), (_, segmented_fwhm) in zip(median_y_data, fwhm_data):
        with fits.open(file_path) as hdul:
            ...
            for x in range(img.shape[1]):
                ...
                lower_outer = int(median_y - (fwhm_outer / 2) * fwhm_rep)
                upper_inner = int(median_y + (fwhm_inner / 2) * fwhm_rep)
                upper_outer = int(median_y + (fwhm_outer / 2) * fwhm_rep)
...
def wavelength_equation(img_type, new_folder_path, sigma=2, prominence_factor=0.1, plotCheck=False, resultPrint=False):
    ...
    for list_file in list_files:
        ...
        for arc_file in arc_files:
            ...
            # 피크 찾기
            prominence = max(smoothed_data) * prominence_factor # 최고 피크의 N% 이내 값까지만 추출
            peaks, properties = find_peaks(smoothed_data, prominence=prominence)
            ...
            # 시각화 (plotCheck=True일 때)
            if plotCheck:
                plt.figure(figsize=(10, 5))

                # 원본 스펙트럼 + 피크 위치 표시
                plt.plot(smoothed_data, label="Smoothed Spectrum", color="blue")
                plt.scatter(peaks, smoothed_data[peaks], color="red", marker="o", label="Peaks", zorder=3)

                plt.xlabel("Pixel Index")
                plt.ylabel("Intensity")
                plt.title(f"Detected Peaks in {arc_file}")
                plt.legend()
                plt.grid(alpha=0.3)
