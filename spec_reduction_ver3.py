...
extract_n = 100              # extraction y-coordinate number
dH = 5                      # extract (y_med - H) ~ (y_med + H)
...
for img_type in ['piser', 'tcrb']:
    median_y_data = spf.extract_spectrum_y_coordi(img_type, new_folder_path, extract_n)
    spf.extract_spectrum_value(img_type, new_folder_path, dH, median_y_data, LV = True, PV = True)
    print(f'Extracted : {img_type}')
print('Obj, Std Extractions are completed')

for img_type in ['arc']:
    spf.extract_spectrum_value_from_center(img_type, new_folder_path, dH, LV = True, PV = True)
    print(f'Extracted : {img_type}')
print('Arc Extractions are completed')
