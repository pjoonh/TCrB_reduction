...
def extract_spectrum_y_coordi(img_type, new_folder_path, n):
    ...
    for file_path in file_paths:
        ...
        with fits.open(file_path) as hdul:
            ...
            x_positions = np.linspace(0, img.shape[1] - 1, n, dtype=int)  # x positions
            peak_y_positions = []

            # Find the peak y-coordinate for each x position
            for x in x_positions:
                column_values = img[:, x]  # Get column values
                peak_y = np.argmax(column_values)  # Find peak y-coordinate
...
# 8-2) extract spectrum value
def extract_spectrum_value(img_type, new_folder_path, dH, median_y_data, LV = False, PV = False):
    processed_data = []

    # Process each file in the median_y_data list
    for file_path, median_y in median_y_data:
        with fits.open(file_path) as hdul:
            img = hdul[0].data  # Get the image data

            # Calculate the range based on dH
            ...
            # Append the result as (file_path, 1D array of median values)
            processed_data.append((file_path, median_values))

            # Visualize the image with horizontal lines
            if LV == False:
                pass
            else:
                show_image_with_line(img, median_y, file_path)
                
            # Visualize selected 1D data values
            if PV == False:
                pass
            else:
                show_1d_spectrum(img, median_values, file_path)

    # Prepare the DataFrame for CSV output
    all_data = []
    for file_path, median_values in processed_data:
    ...
    print(f"Extracted data saved as {csv_file_path}")


def show_image_with_line(img, median_y, file_path):
...
def extract_spectrum_value_from_center(img_type, new_folder_path, dH, LV = False, PV = False):
    # Path to the .list file
    list_file_path = os.path.join(new_folder_path, f'geo_{img_type}.list')
    ...
    for file_path in file_paths:
        with fits.open(file_path) as hdul:
            ...
            # Calculate the central y-coordinate of the image
            center_y = img.shape[0] // 2  # Integer division for the middle row

            # Calculate the range based on dH
            lower_bound = max(0, center_y - dH)
            upper_bound = min(img.shape[0] - 1, center_y + dH)
            ...
            if LV == False:
                ...
                show_image_with_line(img, center_y, file_path)
