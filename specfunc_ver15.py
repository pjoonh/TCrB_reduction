...
def save_combined_spectrum_to_csv(img_type, new_folder_path, spectrum_values, extracted_spectrum_list):
    os.makedirs(new_folder_path, exist_ok=True)
    combined_files = []

    for i, ((file_path, spectrum), (arc_file, star_file, extracted)) in enumerate(zip(spectrum_values, extracted_spectrum_list)):
        file_name = os.path.basename(file_path)
        csv_file_path = os.path.join(new_folder_path, f'extrac_{file_name}.csv')

        # 길이 다를 경우 짧은 길이에 맞춰 자름
        min_len = min(len(spectrum), len(extracted))
        indices = np.arange(min_len)
        spectrum = np.array(spectrum[:min_len])
        extracted = np.array(extracted[:min_len])

        # 적분 계산 (사다리꼴 공식)
        integrate_spectrum = np.zeros(min_len)
        integrate_extracted = np.zeros(min_len)

        integrate_spectrum[0] = spectrum[0]  # 첫 번째 점은 그냥 시작값
        integrate_extracted[0] = extracted[0]

        for j in range(1, min_len):
            dx = indices[j] - indices[j - 1]
            integrate_spectrum[j] = 0.5 * (spectrum[j] + spectrum[j - 1]) * dx
            integrate_extracted[j] = 0.5 * (extracted[j] + extracted[j - 1]) * dx
        ...
        df = pd.DataFrame({
            ...
            "Spectrum": spectrum,
            "Extracted": extracted,
            "Integrate_spectrum": integrate_spectrum,
            "Integrate_extracted": integrate_extracted
        })

        df.to_csv(csv_file_path, index=False)
        print(f"Combined spectrum saved to {csv_file_path}")
        combined_files.append(csv_file_path)

    # 리스트 파일 저장
    list_path = os.path.join(new_folder_path, f'extrac_{img_type}_combined.list')
    with open(list_path, 'w') as f:
        f.write('\n'.join(combined_files) + '\n')
    print(f"Combined file list saved to {list_path}")
...
def plot_resampled_spectra_list(wavelength_list, flux_list, wl_step=1.0, wl_start=None, wl_end=None, show=False):
    all_resampled_wavelengths = []
    all_resampled_fluxes = []

    for idx, (wavelengths, fluxes) in enumerate(zip(wavelength_list, flux_list)):
        wavelengths = np.array(wavelengths)
        fluxes = np.array(fluxes)

        # 길이 맞추기
        min_len = min(len(wavelengths), len(fluxes))
        ...
        # 보간 대상 파장 생성
        start = wl_start if wl_start is not None else wavelengths[0]
        end = wl_end if wl_end is not None else wavelengths[-1]
        uniform_wavelengths = np.arange(start, end, wl_step)

        # spectres를 사용한 flux 보존 리샘플링
        uniform_fluxes = spectres(uniform_wavelengths, wavelengths, fluxes)
        ...
        if show:
            ...
            plt.subplot(1, 2, 1)
            plt.plot(wavelengths, fluxes, color='gray', alpha=0.6, label='Original')
            plt.title(f"Original Spectrum #{idx+1}")
            plt.xlabel('Wavelength (Å)')
            plt.ylabel('Flux')
            plt.grid(True)
            plt.legend()
            ...
            plt.subplot(1, 2, 2)
            plt.plot(uniform_wavelengths, uniform_fluxes, color='royalblue', label='Resampled (spectres)')
            plt.title(f"Resampled Spectrum #{idx+1}")
            plt.xlabel('Wavelength (Å)')
            plt.ylabel('Flux')
            plt.grid(True)
            plt.legend()
            ...
            # Flux 보존 확인
            orig_total = np.trapz(fluxes, wavelengths)
            resampled_total = np.trapz(uniform_fluxes, uniform_wavelengths)
            print(f"Spectrum #{idx + 1} - Flux conservation ratio: {resampled_total / orig_total:.6f}")

        all_resampled_wavelengths.append(uniform_wavelengths)
        all_resampled_fluxes.append(uniform_fluxes)

    return all_resampled_wavelengths, all_resampled_fluxes


# 파장 스케일로 변환된 arc 데이터를 csv에 저장장
def save_resampled_data_to_csv(img_type, new_folder_path, new_wavelengths, resampled_flux):
    os.makedirs(new_folder_path, exist_ok=True)

    for i, (wavelengths, fluxes) in enumerate(zip(new_wavelengths, resampled_flux)):
        data = np.column_stack((wavelengths, fluxes))
        df = pd.DataFrame(data, columns=["Wavelength", "Flux"])
        filename = os.path.join(new_folder_path, f"{img_type}_resampled_data_{i}.csv")
